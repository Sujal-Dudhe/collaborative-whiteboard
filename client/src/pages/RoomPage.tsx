...
export default function RoomPage() {
    ...
    const draw = useCallback(() => {
        ...
        const drawShape = (shape: Shape | Partial<Shape>) => {
            ctx.beginPath()
        ...
        if (currentTool === 'select' && selectedShapeId) {
            ...
            if (sel) {
                let x = sel.data.x ?? 0
                let y = sel.data.y ?? 0
                let w = sel.data.width ?? 0
                let h = sel.data.height ?? 0
                ...
                if (sel.shapeId === 'pen') {
                    ...
                    w = (sel.data.text || '').length * size * 0.6
                    h = size
                }
    ...
    // Hit Testing helper
    const findHitShape = (px: number, py: number) => {
    ...
    const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
        ...
        if (currentTool === 'eraser') {
            const hit = findHitShape(coords.x, coords.y)
            if (hit) {
